describe('test lupa password link (not complete format email)', () => {
  it('reset password', () => {
    ...
    cy.get('#email').clear('dany.christian@g');
    cy.get('#email').type('dany.christian@g');
    cy.get('@buttonKirimLinkReset').click();

    //failed result
    cy.get('.text-red-500').as('alertMsg')
    cy.get('@alertMsg').should('be.visible');
    cy.screenshot('SS test lupa password link (not complete format email)')
