...
test.describe('Check All Page Navigation Tests', () => {
  ...
  test('access conversation team', async ({ page }) => {
    ...
    test.skip((await teamNav.count()) === 0, 'No visible team navigation found in current environment');
    await teamNav.click();
  ...
  test('access profile menu', async ({ page }) => {
    const profileButton = page.locator('button').filter({ has: page.locator('svg') }).last();
