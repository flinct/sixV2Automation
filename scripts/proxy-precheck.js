...
async function main() {
  ...
  const timeoutMs = envInt('TIMEOUT_MS', 8000);
  ...
  console.log({ input, output, testUrl, timeoutMs, concurrency, total: list.length, limit });
  ...
  async function worker(wid) {
    while (true) {
      ...
      if (done % logEvery === 0) {
        console.log(`[progress] done=${done}/${list.length} pass=${pass} fail=${fail}`);
      }
    }
  }

  const workers = Array.from({ length: Math.min(concurrency, list.length) }, (_, k) => worker(k + 1));
