...
function renameLatestLogToFinalReport() {
  ...
  const files = fs
    .readdirSync(logsDir)
    .filter((f) => f.startsWith("monitoringSAP_log_") && f.endsWith(".json"))
    .sort((a, b) => {
  ...
  console.log(
    `✅ ${latestFile} berhasil disalin ke final-report sebagai monitoringSAP_log_latest.json`
  );
}
