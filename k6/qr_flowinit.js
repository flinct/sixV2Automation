 ...
 * - ACCOUNT_CHANNEL_INDEX: fallback index from list response (default 0)
...
const ACCOUNT_CHANNEL_LIST_PATH = envStr("ACCOUNT_CHANNEL_LIST_PATH", "api/account-channel");
const INIT_PATH = envStr("INIT_PATH", "api/account-channel/instance");
const QR_PATH_TEMPLATE = envStr("QR_PATH_TEMPLATE", "api/account-channel/instance/qr/{id}");
...
function pickAccountChannelIdFromList(json) {
  const items = normalizeAccountChannelItems(json);
  if (!items.length) return "";

  if (ACCOUNT_CHANNEL_NAME) {
    const found = items.find((x) => x?.name === ACCOUNT_CHANNEL_NAME || x?.data?.name === ACCOUNT_CHANNEL_NAME);
    if (found) return found?.id || found?._id || found?.data?.id || "";
  }

  if (ACCOUNT_CHANNEL_PHONE) {
    const found = items.find(
      (x) => x?.phoneNumber === ACCOUNT_CHANNEL_PHONE || x?.data?.phoneNumber === ACCOUNT_CHANNEL_PHONE,
    );
    if (found) return found?.id || found?._id || found?.data?.id || "";
  }

  const picked = items[Math.max(0, Math.min(ACCOUNT_CHANNEL_INDEX, items.length - 1))];
  return picked?.id || picked?._id || picked?.data?.id || "";
}
...
function getAccountChannelId(token) {
  ...
  const url = joinUrl(API_BASE, ACCOUNT_CHANNEL_LIST_PATH);
  ...
  const json = res.json();
  const id = pickAccountChannelIdFromList(json);
  check(id, { "account-channel id resolved": (v) => typeof v === "string" && v.length > 0 });
  return id;
}
